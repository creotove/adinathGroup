...
const Index = () => {
  ...
  const handleSubmit =  useCallback(
    async (e) => {
      ...
      const res = await axios.post('api/v1/user/requestForApproval', { ...data })
      if (res.data.success) {
  ...
  const fetchUserRolePrice = useCallback(async () => {
    try {
      setLoading(true)
      const res = await axios.get('api/v1/user/fetchUserRolePrice')
      if (res.data.success) {
        setRole(user.role)
  ...
  const fetchTransactioId = useCallback(async () => {
    try {
      ...
      const res = await axios.post('api/v1/user/fetchTransactionId', { userId })
      if (res.data.success) {
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <Header className="text-light h4 text-center">
            <div className="my-3">
              {' '}
              Hello <span className="text-success">{user.name}</span> Your Account is pending for
              approval!
            </div>
          </Header>
          <Container
                    ...
                    >
                      ...
                      <Row className="mb-3 col-12">
                        ...
                        <Form.Group as={Col} className="col-6" controlId="formLastName">
                          ...
                          <Form.Control type="text" className="col-6" value={`${rolePrice} rs.`} disabled />
                        </Form.Group>
                      </Row>
                      <Row>
                        <Form.Group as={Col} className="col-12 col-md-6 col-lg-6" controlId="formLastName">
